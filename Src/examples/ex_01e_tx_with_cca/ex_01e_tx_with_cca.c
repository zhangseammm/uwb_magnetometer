...
#define INITIAL_BACKOFF_PERIOD 400 /* This constant would normally smaller (e.g. 1ms), however here it is set to 400 ms so that
                                    * user can see (on UART) the report that the CCA detects a preamble on the air occasionally.
...
int dw_main(void)
{
    /* String used to display measured distance (16 characters maximum). */
    char str[17] = {0};
    ...
    while(1)
    {
        /* local variables for UART output, this holds the result of the most recent channel assessment y pseudo CCA algorithm,
        * 1 (channel is clear) or 0 (preamble was detected) */
        ...
        sprintf(str, "CCA=%d   %d  ", channel_clear, tx_sleep_period);
        stdio_write(str);
